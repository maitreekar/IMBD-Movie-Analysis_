...
USING (movie_id)
INNER JOIN  genre g
USING (movie_id), top_genre
WHERE g.genre IN (top_genre.genre) AND r.avg_rating>8
GROUP BY n.name 
ORDER BY count(*) DESC 
) SELECT * FROM
top_directors
...
ROW_NUMBER() OVER (ORDER BY COUNT(movie_id) DESC) AS actor_rank
FROM ratings r1 
...
INNER JOIN movie m 
ON r.movie_id=m.id
WHERE category='actor' AND m.country='India'
GROUP BY n.name
...
INNER JOIN movie m 
ON r.movie_id=m.id
WHERE category='actress' AND m.country='India' AND languages LIKE "%Hindi%"
GROUP BY n.name
